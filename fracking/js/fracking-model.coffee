class FrackingModel extends ABM.Model
  ...
  setupPatches: ->
    ...
    # this is just a common term found in several of the boundary functions
    f = (c, x) => Math.sin(degToRad(c*x - (@width / 4)))

    waterLowerDepth = memo (x) =>
      @waterDepth + @height * f(0.6, x) / 160
    
    rock1LowerDepth = memo (x) =>
      x * rock1Angle + @rock1Depth + @height * -f(0.3, x) / 160

    rock2LowerDepth = memo (x) =>
      x * rock2Angle + @rock2Depth + @height * f(0.4, x) / 160

    rock3LowerDepth = memo (x) => 
      @rock3Depth + @height * f(0.2, x) / 160

    shaleUpperDepth = memo (x) =>
      @oilDepth + @height * Math.sin(degToRad(shaleUpperModifier * x)) / 30

    shaleLowerDepth = memo (x) =>
      @baseDepth + @height * 0.9 * -Math.sin(degToRad(shaleLowerModifier * x + 45)) / 50 + p.x / 10
