...
;In this profile view, patches are either soils layers or sky.  Soils have a bunch of properties that need thier own variables:
...
;patches own variable zone-num is 1 if pxcor is on left or 2 if on right
...
;  it possible to turn the color of eroded patches on and off on use command. 
patches-own [ depth erode-direction eroded? zone-num ]
...
;climate-dta is a data structure for holding static information about climate.  It uses the <turtles-own> variables.
...
breed [plants plant]
plants-own [name state duration rate day-ct annual? in-rows? germinate-day start-x start-y p-initial-size p-durations p-rates p-object is-wintering-over? root-num ]
...
globals [
  ...
  skycolor       ;a bunch of constants to eliminate magic number in the code.  These are set in the setup routine.
  soilcolor
  rainbow-colors
  tree-colors
  root-colors
...
to setup
  ...
  set color-layers? false
  ask patches [if depth >= 0 and depth < 6 [set pcolor soilcolor - 2]]  ;mark topsoil
...
;sets the "depth" patch variable, sky in -1, top layer is 0, 2nd layer is 1, etc.
;  this function uses a emergent algorithm, so to set the depth of the whole soil thickness this need to be run many time. Alternatively,
...
to erode [precip] ;erosion is a function of monthly preciptation
  ...
  let erosion-set patches with [ (pcolor != skycolor) and (count (neighbors with [pcolor != skycolor]) >= 1) and (count neighbors with [ pcolor = skycolor]) >= 3]
  ...
  let surface-list sort-on [ (- count neighbors with [pcolor = skycolor] ) ] erosion-set
        ...
        [
          ...
          [
            ...
            [
              ...
              let erosion-calculated erosion-probability * (precip / 400) * ((0.35 * slope-contribution) + (0.65 * (1 - vegetation-contribution)))
              ...
              [
                let save-zone 0             ;save the zone number of the patch so it can be assigned to the new postion
                ask myself [
                  set save-zone zone-num
                ...
                ask one [
                  ...
                  set pcolor soilcolor       ; set new-soil-patches (patch-set new-soil-patches one)
                  ifelse pxcor = min-pxcor 
                  [
                    set rand-x 1
                  ]
...
to-report soy-data-object ;creates a hidden turtle of breed plant-data that holds the growth data 
  ...
  [
    ...
    set r-colors [32 33 34 35 36]
    set p-initial-size 2
    set germinate-period [75 90]
    set p-durations  [ 35 70 35 35 30 ]                 ;more realistic for trees: (list 60 3600 3600 360 720) 
    set p-rates (list 0.005 0.015 0.0001 -0.04 -0.02)     ;
    set r-initial-size 2
    set r-durations p-durations
    set r-rates (list 0.005 0.015 0.0001 -0.04 -0.02)
...
to-report grass-data-object ;creates a hidden turtle of breed plant-data that holds the growth data 
  ...
  [
    ...
    set p-durations  [ 35 70 360 35 30 ]                 
    set p-rates (list 0.005 0.015 0.00002 -0.04 -0.02)   
    set r-initial-size 2
    set r-durations p-durations
    set r-rates (list 0.005 0.015 0.00002 -0.04 -0.02)
    set winters-over? true
    set withers? true
...
to-report soil-surface-y [ x ]   ;report the pycor value corresponding to the x value; it uses a local average hight of the soil surrounding the x value
  let delta 3
...
to-report random-xcor-in-zone [ z ]  
  if z = 1  [report round random-between min-pxcor 0]
  if z = 2  [report round random-between 0 max-pxcor]
...
to-report next-xcor-in-zone [ z ]
  ...
  [
    ...
    let which-zone item (z - 1) zone-plant-count  ;seperate out the right list
    let tot-num  item 0 which-zone
    let num item 1 which-zone
    let start-at ifelse-value (z = 1) [min-pxcor][0]
    let delta zone-width / tot-num
    let xpos start-at + (delta / 2) + (delta * num)
    set num num + 1
    set which-zone list tot-num num
    set zone-plant-count replace-item (z - 1) zone-plant-count which-zone
    report xpos
  ]
  [
    report 1000   ;error condition will throw failure
  ...
  if not skip-this-seed?
    [
      create-roots 1 [
        ...
        setxy x y - 1
        set root-who who
        set plant-num plant-who    ;same the who number of the root so it can be saved by the plant
      ]
...
to set-annual-land-management-plan   
  ...
  if (day mod year-duration = 0)     ;execute only once per year
  [
    set management-plan-for-year management-plan                                 ;management plan will only change on Jan 1.
    if management-plan-for-year = "Zones 1 and 2: Bare Soil" []    ;do nothing
    if management-plan-for-year = "Zones 1 and 2: Forest" 
      [
        set germinate-day-list-zone-1 []
        set germinate-day-list-zone-2 []
        let i 0
        while [i < 15]
        [
          set germinate-day-list-zone-1 lput (round random-between (first [germinate-period] of Td) (last [germinate-period] of Td)) germinate-day-list-zone-1
          set germinate-day-list-zone-2 lput (round random-between (first [germinate-period] of Td) (last [germinate-period] of Td)) germinate-day-list-zone-2
          set i i + 1
        ]
      ]
    if management-plan-for-year = "Zone 1: Forest; Zone 2: Bare Soil"
    [
        set germinate-day-list-zone-1 []
        set germinate-day-list-zone-2 []
        let i 0
    ...
    if management-plan-for-year = "Zone 1: Forest; Zone 2: Grass"
    [
        set germinate-day-list-zone-1 []
        set germinate-day-list-zone-2 []
      let i 0
      ...
      while [i < 40]
      [
         set germinate-day-list-zone-2 lput (round random-between (first [germinate-period] of Gd) (last [germinate-period] of Gd)) germinate-day-list-zone-2        
         set i i + 1
      ]         
    ]
    if management-plan-for-year = "Zones 1 and 2: Wheat"
    [
      set germinate-day-list-zone-1 []
      set germinate-day-list-zone-2 []
      set zone-plant-count [ [20 0] [20 0] ]
      let i 0
      while [i < 20]
        [
          set germinate-day-list-zone-1 lput (round random-between (first [germinate-period] of Wd) (last [germinate-period] of Wd)) germinate-day-list-zone-1
          set germinate-day-list-zone-2 lput (round random-between (first [germinate-period] of Wd) (last [germinate-period] of Wd)) germinate-day-list-zone-2
          set i i + 1
        ]
      set germinate-day-list-zone-1 sort germinate-day-list-zone-1
      set germinate-day-list-zone-2 sort germinate-day-list-zone-2
    ]
    if management-plan-for-year = "Zone 1: Forest, Zone 2: Wheat"
    [
      set germinate-day-list-zone-1 []
      set germinate-day-list-zone-2 []
      set zone-plant-count [ [15 0] [30 0] ]
      let i 0
      while [i < 15]
        [
          set germinate-day-list-zone-1 lput (round random-between (first [germinate-period] of Td) (last [germinate-period] of Td)) germinate-day-list-zone-1
    ...
    if management-plan-for-year = "Rotate Wheat and Soy"
    [
      set germinate-day-list-zone-1 []
      set germinate-day-list-zone-2 []
      set zone-plant-count [ [30 0] [30 0] ]
      let even-year ifelse-value (year mod 2 = 0) [Sd][Wd]
      let odd-year ifelse-value (year mod 2 = 0) [Wd][Sd]
      let i 0
      while [i < 30]
        [
          set germinate-day-list-zone-1 lput (round random-between (first [germinate-period] of even-year) (last [germinate-period] of even-year)) germinate-day-list-zone-1
          set germinate-day-list-zone-2 lput (round random-between (first [germinate-period] of odd-year) (last [germinate-period] of odd-year)) germinate-day-list-zone-2        
          set i i + 1
        ]
    ]
  ]
...
to manage-the-land
  ...
  if management-plan-for-year = "Zones 1 and 2: Forest" 
    [
      ...
      if (member? day-of-year germinate-day-list-zone-1)
      [
        foreach germinate-day-list-zone-1 [if ? = day-of-year [make-a-seed day-of-year 1 Td (tree-data-randomizer Td) ]]
      ]
      if (member? day-of-year germinate-day-list-zone-2)
      [
        foreach germinate-day-list-zone-2 [if ? = day-of-year [make-a-seed day-of-year 2 Td (tree-data-randomizer Td) ]]
      ]      
    ]
  if management-plan-for-year = "Zone 1: Forest; Zone 2: Bare Soil"
  [
  ...
  if management-plan-for-year = "Zones 1 and 2: Wheat"
    [
      ...
      if (member? day-of-year germinate-day-list-zone-1)
      [
        foreach germinate-day-list-zone-1 [if ? = day-of-year [ make-a-seed day-of-year 1 Wd (wheat-data-randomizer Wd)] ]
      ]
      if (member? day-of-year germinate-day-list-zone-2)
      [
        foreach germinate-day-list-zone-2 [if ? = day-of-year [make-a-seed day-of-year 2 Wd (wheat-data-randomizer Wd)] ]
      ]            
    ]
  if management-plan-for-year = "Zone 1: Forest, Zone 2: Wheat"
    [
    ...
    [
      ...
      if (member? day-of-year germinate-day-list-zone-2)
      [
        foreach germinate-day-list-zone-2 [if ? = day-of-year [make-a-seed day-of-year 2 Gd (grass-data-randomizer Gd)]  ]
      ]      
    ]
    if management-plan-for-year = "Rotate Wheat and Soy"
    [
      let seed-obj-1 ifelse-value (year mod 2 = 0) [soy-data-object][wheat-data-object]
      let rand-1  ifelse-value (year mod 2 = 0) [soy-data-randomizer soy-data-object][wheat-data-randomizer wheat-data-object]
      let seed-obj-2 ifelse-value (year mod 2 = 0) [wheat-data-object][soy-data-object]
      let rand-2 ifelse-value (year mod 2 = 0) [wheat-data-randomizer wheat-data-object][soy-data-randomizer soy-data-object]
      if (member? day-of-year germinate-day-list-zone-1)
      [
        ;show (word "zone 1: " germinate-day-list-zone-1)
        foreach germinate-day-list-zone-1 [if ? = day-of-year [ make-a-seed day-of-year 1 seed-obj-1 rand-1 ]]
      ]
      if (member? day-of-year germinate-day-list-zone-2)
      [
        ;show (word "zone 2: " germinate-day-list-zone-2)
        foreach germinate-day-list-zone-2 [if ? = day-of-year [make-a-seed day-of-year 2 seed-obj-2 rand-2 ]]
      ]      
    ]
end


to-report temperature-rate-factor [ r plant-data-object ] ;;the return value modifies the growth rate depending on temperature
  ...
  if ave-month-temperature > min-t and ave-month-temperature <= opt-t  ;growth in proportion to difference of min growth temp and optimal temp
   [
    ...
    report r * (1 - (present-span / plant-span))
   ]
...
to grow-all [ plant-breed ]
  ...
  [
    ...
    ask plant-breed [ 
      ...
      let local-optimal-time item 1 local-card-temps
      ...
      [  
        ...
        [
          set plants-rate temperature-rate-factor (item plants-state local-p-rates) local-p-data
          ;show (word "plant rate: " plants-rate)
          set delta plants-size * plants-rate
          ...
          set delta roots-size * plants-rate
...
Management-Plan
Management-Plan
"Zones 1 and 2: Bare Soil" "Zones 1 and 2: Forest" "Zone 1: Forest; Zone 2: Bare Soil" "Zone 1: Forest; Zone 2: Grass" "Zones 1 and 2: Wheat" "Zone 1: Forest, Zone 2: Wheat" "Rotate Wheat and Soy"
5
...
404
soil ct
count patches with [pcolor != skycolor]
17
1
11
